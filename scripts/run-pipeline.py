...
def _state_path(project: str) -> Path:
    return REPO_ROOT / "projects" / project / "pipeline-state.json"
...
def _prompt_for_phase(phase: str, project: str, state: dict) -> str:
    ...
    prompts = {
        ...
        "2a-test-plan": (
            ...
            f"2. Read the validation checklist from validation/{task_flow}.md\n"
...
def _extract_task_flow(project: str) -> str | None:
    ...
    handoff = REPO_ROOT / "projects" / project / "prd" / "architecture-handoff.md"
...
def start_pipeline(display_name: str, problem: str | None = None) -> dict:
    ...
    spec = importlib.util.spec_from_file_location("new_project", str(REPO_ROOT / "scripts" / "new-project.py"))
