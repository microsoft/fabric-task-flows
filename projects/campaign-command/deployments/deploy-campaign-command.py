...
def populate_variable_library(ws_id, headers):
    ...
    variables.append({"name": "Project_Name", "type": "String", "value": "campaign-command", "note": "Project name"})
...
def main():
    parser = argparse.ArgumentParser(description="Deploy campaign-command")
    ...
    print()
    print("  Project:   campaign-command")
    print("  Task Flow: lambda")
    print()
    ...
    if args.mode == "single":
        ...
        if args.workspace_id:
            ...
            ws_name = args.workspace or input("  ? Workspace name (Enter = campaign-command): ").strip() or "campaign-command"
            ws_id = ensure_workspace(ws_name, headers, "campaign-command — lambda architecture. Batch ad analytics + real-time social sentiment + exec chatbot for campaign ROI optimization.")
        print()
        print("  -- CAPACITY --")
        ...
        base_name = args.workspace or input("  ? Base workspace name (Enter = campaign-command): ").strip() or "campaign-command"
        envs = ["dev", "ppe", "prod"]
        print()
        ...
        for env in envs:
            ...
            print(f"  {env.upper()}:")
            ws_ids[env] = ensure_workspace(ws_name, headers, "campaign-command — lambda architecture. Batch ad analytics + real-time social sentiment + exec chatbot for campaign ROI optimization.")
        print()
        print("  -- CAPACITY --")
        for env in envs:
            print(f"  {env.upper()}:")
