...
1. **Understand Requirements** - Start from a Discovery Brief when available, then fill gaps:
   ...
   **If a Discovery Brief from `@fabric-advisor` is available:**
   ...
   - Address the "Architectural Judgment Calls" section — these are design trade-offs flagged for your expertise (not customer-answerable facts)
...
- **Note:** A deterministic diagram is also auto-generated by `scripts/diagram-gen.py` at sign-off from the items/waves YAML. The review-prescan validates your diagram for balanced box characters and item coverage. Your hand-drawn diagram is the primary one for the review phase; the scripted one supplements it at sign-off.
