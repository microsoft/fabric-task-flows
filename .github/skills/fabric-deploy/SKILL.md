...
5. `references/fabric-cli-commands.md` — CLI reference (used for post-deploy validation via `fab exists`)
...
3. **Verify creation:** Run `fab exists` for each item (Fabric CLI is used for validation only)
