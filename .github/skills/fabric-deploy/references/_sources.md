# Reference Sources

These references are bundled directly in this skill:

- `references/deployment-patterns.md` — ⚠️ DEPRECATED — legacy `fab mkdir` patterns (retained for reference)
- `references/parallel-deployment.md` — wave parallelization patterns
- `references/rollback-protocol.md` — failure recovery procedures
- `references/fabric-cli-commands.md` — ⚠️ DEPRECATED — legacy `fab` CLI reference (retained for reference)
- `references/prerequisites.md` — pre-deployment setup guide
- `references/verified-item-definitions.md` — authoritative item type capabilities and deploy status
- `scripts/deploy-script-gen.py` — Generates fabric-cicd workspace directory, config.yml, and deploy script from architecture handoff

External shared references (load from `_shared/`):

- `_shared/learnings.md` — operational gotchas (check before deploying)
